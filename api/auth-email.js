// api/auth-email.js
// Uses Brevo to send auth emails with full diagnostic logging.
// Required env vars: SUPABASE_URL, SUPABASE_SERVICE_KEY, BREVO_API_KEY, NEXT_PUBLIC_APP_URL
...
export default async function handler(req, res) {
  ...
  if (missing.length) {
    console.error(`[auth-email] Missing env vars: ${missing.join(", ")}`);
    return res.status(500).json({ error: `Missing env vars: ${missing.join(", ")}` });
  }

  try {
    // Step 1: Generate Supabase auth link
    ...
    console.log(`[auth-email] Supabase link generated OK`);
